...
import com.apk.reminder.Adapter.adapterDosen;
...
import java.util.Map;
 ...
 public class recycleDosen extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_recycle_dosen);



         recyclerView = findViewById(R.id.id_recycle_dosen);
         linearLayoutManager = new LinearLayoutManager(this);
         recyclerView.setLayoutManager(new LinearLayoutManager(getApplicationContext()));

         dBuser = new DBuser( recycleDosen.this );
         datalist = dBuser.getData( FieldUser.NAMA_TABLE );
         id = datalist[0];
         name = datalist[1];

         getDataPenginputan(getApplicationContext());



    }

    void getDataPenginputan( Context context){
       // adapterDosen =new adapterDosen(list,context);
        recyclerView.setAdapter(adapterDosen);


        StringRequest stringRequest = new StringRequest(Request.Method.POST, "http://anr.my.id/APIR/tampil-card-dosen", new Response.Listener<String>() {
            @Override
